...
def get_match_predictions(match_id: int, db: Session = Depends(get_db)):
    ...
    predictions = db.query(Prediction, User.username).join(
        User, Prediction.user_id == User.id).filter(Prediction.match_id == match_id).all()
...
def create_prediction(prediction_in: schemas.PredictionCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if datetime.utcnow() >= match.match_date:
        raise HTTPException(
            status_code=400, detail="Predictions are locked for this match")
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="Prediction already exists for this match")
...
def update_prediction(match_id: int, prediction_in: schemas.PredictionUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if not db_prediction:
        raise HTTPException(
            status_code=404, detail="No prediction found for this match")
...
def get_leaderboard(db: Session = Depends(get_db)):
    ...
    predictions = db.query(Prediction, Match).join(
        Match, Prediction.match_id == Match.id).filter(Match.status == "finished").all()
    ...
    for pred, match in predictions:
        ...
        if username not in user_stats:
            user_stats[username] = {
                "exact": 0, "winner": 0, "home": 0, "away": 0, "total": 0}

        stats = user_stats[username]

        # Get the multiplier for this match's phase
        multiplier_obj = db.query(PhaseMultiplier).filter(
            PhaseMultiplier.id == match.phase_id).first()
    ...
    leaderboard.sort(key=lambda x: (
        x["total_points"], x["exact_score_pts"], x["winner_pts"]
    ), reverse=True)
    return leaderboard
